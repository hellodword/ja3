...
func makeProxyDialer(proxyURL *url.URL, cfg *utls.Config, clientHelloID *utls.ClientHelloID, timeout time.Duration) (proxy.Dialer, error) {
	...
	case "http":
		proxyDialer, err = ProxyHTTP("tcp", proxyAddr, auth, timeout, proxyDialer)
	case "https":
		...
		proxyDialer, err = ProxyHTTPS("tcp", proxyAddr, auth, timeout, proxyDialer, cfgClone, clientHelloID)
	default:
...
func NewUTLSRoundTripper(clientHelloID *utls.ClientHelloID, cfg *utls.Config, proxyURL *url.URL, timeout time.Duration) (http.RoundTripper, error) {
	proxyDialer, err := makeProxyDialer(proxyURL, cfg, clientHelloID, timeout)
	if err != nil {
		return nil, err
	}
