...
func TestUTLSClientHello(t *testing.T) {
	...
	rt, err := NewUTLSRoundTripper(&utls.HelloIOS_11_1, &utls.Config{InsecureSkipVerify: true, ServerName: "localhost"}, nil, time.Second*5)
	if err != nil {
		panic(err)
	}
...
func TestUTLSServerName(t *testing.T) {
	var clientHelloID = &utls.HelloFirefox_63
	...
	rt, err := NewUTLSRoundTripper(clientHelloID, &utls.Config{InsecureSkipVerify: true}, nil, time.Second*5)
	if err != nil {
		panic(err)
	}
	buf, err := clientHelloResultingFromRoundTrip(t, "127.0.0.1", rt.(*UTLSRoundTripper))
	...
	// the ServerName.
	rt, err = NewUTLSRoundTripper(clientHelloID, &utls.Config{InsecureSkipVerify: true, ServerName: "test.example"}, nil, time.Second*5)
	if err != nil {
		panic(err)
	}
	buf, err = clientHelloResultingFromRoundTrip(t, "127.0.0.1", rt.(*UTLSRoundTripper))
...
func TestUTLSHTTPWithProxy(t *testing.T) {
	...
	for _, proxyURL := range []url.URL{
		...
		rt, err := NewUTLSRoundTripper(&utls.HelloFirefox_63, &utls.Config{InsecureSkipVerify: true}, &proxyURL, time.Second*5)
		if err != nil {
			panic(err)
		}
