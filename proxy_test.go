...
func TestProxyHTTPCONNECT(t *testing.T) {
	req, err := requestResultingFromDialHTTP(t, func(addr net.Addr) (*httpProxy, error) {
		return ProxyHTTP("tcp", addr.String(), nil, time.Second*5, proxy.Direct)
	}, "tcp", testAddr)
	if err != nil {
		panic(err)
	}
...
func TestProxyHTTPProxyAuthorization(t *testing.T) {
	...
	req, err := requestResultingFromDialHTTP(t, func(addr net.Addr) (*httpProxy, error) {
		return ProxyHTTP("tcp", addr.String(), auth, time.Second*5, proxy.Direct)
	}, "tcp", testAddr)
	if err != nil {
		panic(err)
	}
...
func TestProxyHTTPSCONNECT(t *testing.T) {
	req, err := requestResultingFromDialHTTPS(t, func(addr net.Addr) (*httpProxy, error) {
		return ProxyHTTPS("tcp", addr.String(), nil, time.Second*5, proxy.Direct, &utls.Config{InsecureSkipVerify: true, ServerName: testHost}, &utls.HelloFirefox_Auto)
	}, "tcp", testAddr)
	if err != nil {
		panic(err)
	}
