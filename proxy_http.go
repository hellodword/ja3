...
func (pr *httpProxy) Dial(network, addr string) (net.Conn, error) {
	...
	conn.SetDeadline(pr.deadline)

	err = connectReq.Write(conn)
	if err != nil {
		conn.Close()
		err = fmt.Errorf("proxy write error: %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		conn.Close()
		err = fmt.Errorf("proxy read error: %s", err.Error())
		return nil, err
	}
...
func ProxyHTTP(network, addr string, auth *proxy.Auth, timeout time.Duration, forward proxy.Dialer) (*httpProxy, error) {
	return &httpProxy{
		network:  network,
		addr:     addr,
		auth:     auth,
...
func ProxyHTTPS(network, addr string, auth *proxy.Auth, timeout time.Duration, forward proxy.Dialer, cfg *utls.Config, clientHelloID *utls.ClientHelloID) (*httpProxy, error) {
	return &httpProxy{
		network: network,
		addr:    addr,
		auth:    auth,
